...
class Import
  ...
  def json_load(data)
    ...
    if data["data_base64"]
      ...
      option = "d"
      option = "D" if RUBY_PLATFORM == "x86_64-darwin15"
      cmd = "rm -rf #{File.join(DATA_FOLDER, "*")} && cat #{DATA_BASE64_FILE} | base64 -#{option} | tar xzC #{DATA_FOLDER}"
    ...
    ActiveRecord::Base.transaction do
      APPLICATION_MODELS.each do |model|
        ...
        unless max_id <= next_seq
          ActiveRecord::Base.connection.execute("alter sequence #{seq_name} start with #{max_id} restart")
          next_seq = ActiveRecord::Base.connection.select_values("select nextval('#{seq_name}')")[0].to_i
